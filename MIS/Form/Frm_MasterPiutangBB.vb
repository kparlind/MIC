...
Public Class Frm_MasterPiutangBB
    ...
    Private Sub Frm_MasterPiutangBB_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        InitField()
        Status_Proses = ""
        SetToolTip()
        SetEnabled(False)
    ...
    Private Sub loadCbsearch()
        Try
            ...
            Cmd.CommandText = "exec sp_Retreive_FieldList 'Master_Piutang_BB'"
    ...
    Private Sub LoadGrid()
        ...
        Try
            ...
            Cmd.Connection = Conn
            Cmd.CommandType = CommandType.Text
            Cmd.CommandText = "exec sp_Retrieve_Master_Piutang_BB"
            DA.SelectCommand = Cmd
            DA.Fill(dtTemp)

            GridPiutangBB.DataSource = dtTemp
    ...
    Private Sub SetBackColor_Field(ByVal proses As String)
        If proses = "READ" Then
            txtCustID.BackColor = Color.LemonChiffon
            txtNamaCust.BackColor = Color.DarkGray
    ...
    Private Sub txtCustID_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtCustID.KeyDown
        If e.KeyCode = Keys.F1 Then
            Try
                CallandGetSearchData("Select cust_id,nama,alamat,deskripsi,telp from Master_Customer where active_flag = 'Y'", "Cust_ID", "Nama", "Alamat", "Deskripsi", "Telp")
                ...
                If frmSearch.txtResult1.Text.ToString.Trim <> "" Then
                    ...
                    If dt_piutang.Rows.Count > 0 Then
                        ...
                        txtNamaCust.Text = dt_piutang.Rows(0).Item("Nama").ToString
                        TxtValue.Focus()
                    Else
        ...
        ElseIf e.KeyCode = Keys.Enter Then
            If txtCustID.Text.Trim <> "" Then
                Try
                    ...
                    End If
                    dt_piutang.Clear()
                    Cmd.CommandText = "EXEC sp_Retrieve_Master_Customer '" & txtCustID.Text.Trim & "'"
                    DA.SelectCommand = Cmd
                    DA.Fill(dt_piutang)

                    If dt_piutang.Rows.Count > 0 Then
                        txtNamaCust.Text = dt_piutang.Rows(0).Item(GlobalVar.Fields.Item_Name).ToString
    ...
    Private Sub GetItemData(ByVal ItemId As String)
        Try
            If Conn.State = ConnectionState.Closed Then
                Conn.Open()
            End If
            Cmd.CommandText = "EXEC sp_Retrieve_Master_Customer_ByKey '" + ItemId + "'"
            DA.SelectCommand = Cmd
            DA.Fill(dt_piutang)
    ...
    Private Sub btn_cari_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_cari.Click
        ...
        Try
            ...
            Cmd.Connection = Conn
            Cmd.CommandType = CommandType.Text
            Cmd.CommandText = "exec Search_Master_List '" & cb_searchBaseon.SelectedValue.ToString & "', '" & txt_SearchData.Text.ToString.Trim & "', 'Master_Piutang_BB'"
            DA.SelectCommand = Cmd
            DA.Fill(dtTemp)
    ...
    Private Sub GridPiutangBB_CellClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles GridPiutangBB.CellClick
        Dim i As Integer
        Dim Month As String
        Dim Year As String
        i = GridPiutangBB.CurrentRow.Index

        Year = Microsoft.VisualBasic.Left(GridPiutangBB.Item(0, i).Value, 4)
        Month = Microsoft.VisualBasic.Right(GridPiutangBB.Item(0, i).Value, 2)
        cbxPeriod.SelectedItem = Year & "-" & Month
        txtCustID.Text = GridPiutangBB.Item(1, i).Value
        If GridPiutangBB.Item(2, i).Value = 0 Then
            TxtValue.Text = GridPiutangBB.Item(3, i).Value
            rbCredit.Checked = True
        Else
            TxtValue.Text = GridPiutangBB.Item(2, i).Value
            rbDebet.Checked = True
        End If
        ...
        If txtCustID.Text.Trim <> "" Then
            Try
                ...
                If dt_piutang.Rows.Count > 0 Then
                    txtNamaCust.Text = dt_piutang.Rows(0).Item(GlobalVar.Fields.Supp_Name).ToString
            ...
            End Try
        End If
        If GridPiutangBB.Item(4, i).Value = "Y" Then
            rbYes.Checked = True
        Else
            rbNo.Checked = True
        End If
    ...
    Private Sub btnDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDelete.Click
        ...
        If lv_click = "Y" Then
            Try
                ...
                Cmd.CommandText = "exec sp_Delete_Master_Piutang_BB '" & Period & "' ,'" & txtCustID.Text.Trim & "'"

                Cmd.ExecuteNonQuery()
                Insert_Trans_History(txtCustID.Text.Trim, Me.Name, "DELETE") 'Insert History transaction
                Conn.Close()
            Catch ex As Exception
                MessageBox.Show(ex.Message, "Warning", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                Conn.Close()
        ...
        GridPiutangBB.DataSource = Nothing
        LoadGrid()
        SetToolTip()
        InitField()
    End Sub
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        Dim dtTemp As New DataTable
        If txtNamaCust.Text = "" Or txtNamaCust.Text = String.Empty Then
            MessageBox.Show("Nama Customer Can not be Empty !", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            Exit Sub
        End If
        If cbxPeriod.Text = "" Or cbxPeriod.SelectedItem = "" Then
            MessageBox.Show("Period Can not be Empty !", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            Exit Sub
        End If
        If txtCustID.Text = "" Or txtCustID.Text = String.Empty Then
            MessageBox.Show("Period Can not be Empty !", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            Exit Sub
        End If
        ...
        If lv_click = "Y" Then
            Try
                ...
                Cmd.CommandText = "exec sp_Update_Master_Piutang_BB '" & Period & "', '" & txtCustID.Text.Trim & "', '" _
                ...
                Insert_Trans_History(txtCustID.Text.Trim, Me.Name, "UPDATE") 'Insert History transaction
        ...
        Else
            Try
                ...
                Cmd.CommandText = "exec sp_Retrieve_Master_Piutang_BB_ByPK '" & Period & "', '" & txtCustID.Text.Trim & "'"
                ...
                Else
                    ...
                    Cmd.CommandText = "exec sp_Insert_Master_Piutang_BB '" & Period & "', '" & txtCustID.Text.Trim & "', '" _
                                    & lv_debet & "' ,'" & lv_credit & "' ,'" _
                                 & userlog.UserName & "',  '" & lv_active & "'"
                    ...
                    'UpdateSerial(Me.Name, GetCurrMonth, GetCurrYear, LastSerial, userlog.UserName) 'Update Serial
                    Insert_Trans_History(txtCustID.Text.Trim, Me.Name, "INSERT") 'Insert History transaction
                End If
