...
Public Class FrmPencairanGiroPiutang
    ...
    Private Sub FrmPencairanGiroPiutang_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        Conn.ConnectionString = connectStr 'Set Default connection
        Cmd.Connection = Conn
        Cmd.CommandType = CommandType.Text
        txt_tglCair.Value = Now
        ...
        If txt_nogiro.Text.Trim <> "" Then
            ...
            Try
                ...
                Cmd.CommandText = "EXEC sp_Retreive_Trans_PencairanGiroPiutang '" + txt_nogiro.Text + "'"
            ...
            txt_pelunasan.Text = dt.Rows(0).Item("PP_No")
    ...
    Private Sub LoadPencairanGiro(ByVal No As String)
        Try
            ...
            Cmd.CommandText = "EXEC sp_Retreive_Trans_PencairanGiroPiutang '" + No + "'"
            DA.SelectCommand = Cmd
            DA.Fill(dt_hdr)
    ...
    Private Sub txt_nogiro_KeyDown(sender As Object, e As System.Windows.Forms.KeyEventArgs) Handles txt_nogiro.KeyDown
        If e.KeyCode = Keys.F1 Then
            CallandGetSearchData("exec sp_GetNoGiroPiutang", "No_Giro", "PP_No", "Jatuh_Tempo", "jumlah_Bayar", "Account", "tgl_pelunasan")
    ...
    Private Sub btn_save_Click(sender As Object, e As System.EventArgs) Handles btn_save.Click
        ...
        If cmb_status.SelectedIndex = 0 Then
            MessageBox.Show("Status harus dipilih.", "Peringatan", MessageBoxButtons.OK, MessageBoxIcon.Warning, MessageBoxDefaultButton.Button1)
            Exit Sub
        End If
        If txt_bank.Text = "" Then
            MessageBox.Show("Bank harus diisi.", "Peringatan", MessageBoxButtons.OK, MessageBoxIcon.Warning, MessageBoxDefaultButton.Button1)
            Exit Sub
        End If
        ...
        If Status_Proses = "Insert" Then
            ...
            Else
                ...
                dr("PP_No") = txt_pelunasan.Text
                dr("Status") = cmb_status.SelectedItem
                dr("Tgl_Cair") = txt_tglCair.Value.ToString("yyyy-MM-dd")
        ...
        ElseIf Status_Proses = "Update" Then
            ...
            If da IsNot Nothing Then
                ...
                da("PP_No") = txt_pelunasan.Text
                da("Status") = cmb_status.SelectedItem
                da("Tgl_Cair") = txt_tglCair.Value.ToString("yyyy-MM-dd")
                da("Bank") = txt_bank.Text
            End If
        End If
        TextBox1.Text = txt_nogiro.Text
        gvGiro.DataSource = dt_hdr
        Status_Proses = "" 'reset
    ...
    Private Sub ts_save_Click(sender As Object, e As System.EventArgs) Handles ts_save.Click
        If Conn.State = ConnectionState.Closed Then
            Conn.Open()
        End If
        Dim dt As New DataTable
        ...
        Try
            ...
            Try
                ...
                Cmd.CommandText = "select giro_no from trans_pencairangiroPiutang where giro_no = '" + TextBox1.Text + "'"
                DA.SelectCommand = Cmd
                DA.Fill(dt)
            ...
            If dt.Rows.Count > 0 Then
                For Each item As DataRow In dt_hdr.Rows
                    ...
                    Cmd.CommandText = "EXEC sp_Update_transPencairanGiroPiutang '" & item("Giro_NO") & "','" & _
                                             item("Nama_Bank") & "','" & _
                                             item("Tgl_Bayar") & "','" & _
                                             item("Jatuh_Tempo") & "','" & _
                                             item("Nilai_Giro") & "','" & _
                    ...
                    If item("Status") = "Cair" Then
                        ...
                        Cmd.CommandText = "EXEC sp_Insert_Journal_Item '" & JournalNo + "','" & _
                                         "Piutang Giro" & "'," & "0" & "," & item("Nilai_Giro") & ",'" & "" & "','" & userlog.UserName & "'"
                        Cmd.ExecuteNonQuery()
                    ...
                    ElseIf item("Status") = "Tolak" Then
                        ...
                        Cmd.CommandText = "EXEC sp_Insert_Journal_Item '" & JournalNo + "','" & _
                                          "Piutang Giro" & "'," & item("Nilai_Giro") & "," & "0" & ",'" & "" & "','" & userlog.UserName & "'"
                        Cmd.ExecuteNonQuery()
            ...
            Else
                ...
                For Each item As DataRow In dt_hdr.Rows
                    Cmd.CommandText = "EXEC sp_Insert_PencairanGiroPiutang '" & item("Giro_NO") & "','" & _
                                             item("Nama_Bank") & "','" & _
                                             item("Tgl_Bayar") & "','" & _
                                             item("Jatuh_Tempo") & "','" & _
                                             item("Nilai_Giro") & "','" & _
            ...
            FrmPencairanGiroPiutang_Load(Nothing, Nothing)
        Catch ex As Exception
            ObjTrans.Rollback()
            Conn.Close()
            MsgBox(ex.Message)
    ...
    Private Sub btn_edit_Click(sender As Object, e As System.EventArgs) Handles btn_edit.Click
        ...
        txt_nogiro.BackColor = Color.DarkGray
        txt_nogiro.Enabled = False
        txt_NamaBank.Enabled = True
        txt_NamaBank.BackColor = Color.LemonChiffon
        txt_tglCair.Enabled = True
    End Sub
    ...
    Private Sub gvGiro_CellMouseClick(sender As Object, e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles gvGiro.CellMouseClick
        If String.IsNullOrEmpty(gvGiro.Rows(gvGiro.CurrentCell.RowIndex).Cells("Giro_No").Value) = False Then
            'If gvGiro.Rows(gvGiro.CurrentCell.RowIndex).Cells("Status").Value.ToString.Trim = "Cair" Or gvGiro.Rows(gvGiro.CurrentCell.RowIndex).Cells("Status").Value.ToString.Trim = "Tolak" Then
            '    MessageBox.Show("Status Cair/Tolak tidak bisa diedit", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1)
            '    Exit Sub
            'End If
            ...
            txt_pelunasan.Text = gvGiro.Rows(gvGiro.CurrentCell.RowIndex).Cells("PP_No").Value.ToString.Trim
    ...
    Private Sub txt_bank_KeyDown(sender As Object, e As System.Windows.Forms.KeyEventArgs) Handles txt_bank.KeyDown
        If e.KeyCode = Keys.F1 Then
            CallandGetSearchData("Select Account_ID,Account_Name from Master_CoA where Active_Flag = 'Y' and group_payaccount= 'Bank'", "Account_ID", "Account_Name", "", "", "")
            txt_bank.Text = frmSearch.txtResult1.Text.ToString.Trim
        End If
    End Sub

    Private Sub ts_cancel_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        DisableButton(False)
        Enable_Button_Wa(False)
        Clear_Wa()
        SetBackColor_Wa("READ")
    ...
    Private Sub ts_New_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        Enable_Button_Wa(True)
        Enable_Wa(False)
        DisableButton(False)
        ts_save.Enabled = True
